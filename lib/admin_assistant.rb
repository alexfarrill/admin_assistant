...
require 'admin_assistant/show'
require 'admin_assistant/request'
require 'will_paginate'

class AdminAssistant
  attr_reader :custom_column_labels, :form_settings, :index_settings, :show_settings, :model_class
  ...
  def has_one_assoc(name)
    model_class.reflect_on_all_associations.detect { |assoc|
      assoc.macro == :has_one && assoc.name.to_s == name.to_s
    }
  end
  
  def has_many_assoc(name)
    model_class.reflect_on_all_associations.detect { |assoc|
      assoc.macro == :has_many && assoc.name.to_s == name.to_s
    }
  end
  ...
  def column(name)
    column = if file_columns.include?(name)
      ...
      BelongsToColumn.new(belongs_to_assoc, model_class)
    elsif has_one_assoc = has_one_assoc(name)
      HasOneColumn.new(has_one_assoc)
    elsif has_many_assoc = has_many_assoc(name)
      HasManyColumn.new(has_many_assoc)
    else
  ...
  def method_missing(meth, *args)
    request_methods = [:create, :destroy, :edit, :index, :new, :show, :reorder, :update]
  ...
  def url_params(a = action, opts = {})
    { :controller => @controller_class.controller_path, :action => a }.merge(opts)
  end
